...
export class CreditCardLoggerFilter implements LoggerFilter {
  ...
  private readonly BANK_DIGITS: RegExp = new RegExp(/\b\d{11,17}\b/g); // anything that doesn't look like a phone number
  ...
  public redact(toRedact: ToRedact): any {
    ...
    if (strValue.length < 11) return value; // try to avoid unnecessary regex checks
    return strValue
